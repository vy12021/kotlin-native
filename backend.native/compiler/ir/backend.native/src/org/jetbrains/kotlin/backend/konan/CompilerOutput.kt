...
internal fun createOutputProducer(context: Context) {

    val config = context.config.configuration
    val produce = config.get(KonanConfigKeys.PRODUCE)
            ?: throw IllegalArgumentException("Unknown produce type")

    context.backendProducer = when (produce) {
        CompilerOutputKind.DYNAMIC,
        CompilerOutputKind.FRAMEWORK,
        CompilerOutputKind.PROGRAM -> ProgramProducer(context)
        CompilerOutputKind.LIBRARY -> LibraryProducer(context)
        CompilerOutputKind.BITCODE -> BitcodeProducer(context)
    }
}
